      ...
      <div className="sticky top-0 z-[100] glass-panel bg-white/70 backdrop-blur-md border-b border-white/40 shadow-sm">
        <div className="grid px-3 pt-2 pb-1 gap-x-2 gap-y-1" style={{ gridTemplateColumns: '1fr auto' }}>
          <div className="relative min-w-0" ref={storePanelRef}>
            <button onClick={() => { setShowStorePanel(prev => !prev); setShowPartPanel(false); }} className={`w-full flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold smooth-transition active:scale-[0.98] border border-emerald-700/50 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.1)] bg-gradient-to-b from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white ${!selectedStore && 'opacity-60 saturate-50'}`}>
          ...
          <button onClick={onRefreshCloud} disabled={cloudLoading} className={`flex items-center justify-center gap-1 w-[52px] py-1.5 rounded-xl text-xs font-bold border-teal-700/50 border smooth-transition active:scale-95 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.1)] bg-gradient-to-b from-teal-500 to-teal-600 hover:from-teal-400 hover:to-teal-500 text-white disabled:opacity-60 disabled:saturate-50 disabled:pointer-events-none`}><RefreshCw size={13} className={cloudLoading ? 'animate-spin' : ''} /><Cloud size={13} /></button>

          <div className="relative min-w-0" ref={partPanelRef}>
            <button disabled={!selectedStore || partsLoading || storeParts.length === 0} onClick={() => { setShowPartPanel(prev => !prev); setShowStorePanel(false); }} className={`w-full flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border border-blue-700/50 smooth-transition active:scale-[0.98] shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.1)] bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white disabled:opacity-60 disabled:saturate-50 disabled:pointer-events-none`}>
              <Layers size={13} className="text-white/80" />
              {!selectedStore ? <span className="flex-1 text-left truncate">영업점 먼저 선택</span> : partsLoading ? <span className="flex-1 text-left truncate">불러오는 중...</span> : storeParts.length === 0 ? <span className="flex-1 text-left truncate">설정된 파트 없음</span> : selectedPart ? <span className="flex-1 text-left truncate">{selectedPart.name}<span className="text-blue-100 ml-1.5 text-[10px] bg-blue-800/50 px-1 py-0.5 rounded-md">{selectedPart.categories.length}개 조코드</span></span> : <span className="flex-1 text-left truncate">파트를 선택하세요</span>}
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1 w-[52px] py-1.5 rounded-xl text-xs font-bold border border-amber-700/50 smooth-transition active:scale-95 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.1)] bg-gradient-to-b from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-white disabled:opacity-60 disabled:saturate-50 disabled:pointer-events-none`}
          >
            <BookOpen size={13} />
