    ...
    <div className="glass-panel print:hidden relative z-50">
      <div className="px-3 py-2 flex justify-center">
        <div className="flex items-stretch gap-2">
            <div className="border-2 border-dashed border-slate-300/60 rounded-2xl flex flex-col items-center justify-center gap-0.5 bg-white/40 hover:bg-white hover:border-slate-400 hover:shadow-md smooth-transition active:scale-95 cursor-pointer w-[72px] h-[60px]" onClick={() => fileInputRef.current?.click()}>
              <Upload size={24} className="text-slate-500 transition-transform duration-300 group-hover:scale-110" />
              ...
              <div onClick={() => handleDownloadCloudFile(thisWeekCloud?.url, thisWeekCloud?.filename)} className={`border border-white/60 shadow-sm rounded-xl px-3 py-1 bg-white/60 backdrop-blur-sm flex items-center gap-2 smooth-transition ${thisWeekCloud?.exists ? 'hover:bg-white/90 hover:scale-[1.02] cursor-pointer hover:shadow-md' : 'cursor-default'}`} title={thisWeekCloud?.exists ? "클릭하여 원본 엑셀 다운로드" : ""}>
                ...
                {thisWeekCloud?.exists ? (
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold shrink-0 font-mono text-center w-[72px]">{thisWeekCloud.weekKey}</span>
                    <span className="text-[10px] text-slate-400 font-bold shrink-0 text-center w-[68px]">{formatDate(thisWeekCloud.uploadedAt)}</span>
              ...
              <div onClick={() => handleDownloadCloudFile(lastWeekCloud?.url, lastWeekCloud?.filename)} className={`border border-white/60 shadow-sm rounded-xl px-3 py-1 bg-white/60 backdrop-blur-sm flex items-center gap-2 smooth-transition ${lastWeekCloud?.exists ? 'hover:bg-white/90 hover:scale-[1.02] cursor-pointer hover:shadow-md' : 'cursor-default'}`} title={lastWeekCloud?.exists ? "클릭하여 원본 엑셀 다운로드" : ""}>
                ...
                {lastWeekCloud?.exists ? (
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold shrink-0 font-mono text-center w-[72px]">{lastWeekCloud.weekKey}</span>
                    <span className="text-[10px] text-slate-400 font-bold shrink-0 text-center w-[68px]">{formatDate(lastWeekCloud.uploadedAt)}</span>
            ...
            <div className="flex flex-col gap-1 min-w-0 justify-center h-[60px]" style={{ flex: '0 1 360px' }}>
              <div className="relative" ref={storePanelRef}>
                <button onClick={() => { setShowStorePanel(prev => !prev); setShowPartPanel(false); }} className={`w-full flex items-center gap-1.5 px-3 py-1 rounded-xl text-[11px] smooth-transition active:scale-[0.98] border shadow-sm ${selectedStore ? 'bg-emerald-50/90 border-emerald-200/60 text-emerald-800 hover:shadow-md' : 'bg-white/60 border-white/80 text-slate-500 hover:bg-white hover:shadow-md'}`}>
                  ...
                  {selectedStore ? <span className="font-semibold flex-1 text-left truncate"><span className="text-emerald-500 font-mono mr-1.5">{selectedStore.code}</span>{selectedStore.name}</span> : <span className="flex-1 font-bold text-left text-slate-400 text-[10px]">영업점 선택</span>}
                ...
                {showStorePanel && (
                  <div className="absolute left-0 right-0 top-full mt-2 bg-white shadow-[0_20px_40px_rgba(0,0,0,0.15)] rounded-2xl border border-gray-200/50 z-[60] overflow-hidden origin-top animate-in fade-in slide-in-from-top-2">
                    <div className="p-2 border-b border-slate-100/60">
                      <div className="flex items-center gap-2 bg-slate-50/80 rounded-xl px-3 py-2 border border-slate-100"><Search size={14} className="text-slate-400 shrink-0" /><input type="text" value={storeSearch} onChange={(e) => setStoreSearch(e.target.value)} placeholder="코드/영업점명 검색" className="flex-1 bg-transparent text-sm font-bold outline-none placeholder-slate-400 text-slate-700" autoFocus />{storeSearch && <button onClick={() => setStoreSearch("")} className="p-0.5 hover:bg-slate-200 rounded-full transition-colors"><X size={12} className="text-slate-400" /></button>}</div>
                    </div>
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar bg-white">
                      {filteredStores.map(store => {
                        const isSelected = selectedStore?.code === store.code;
                        return (
                          <button key={store.code} onClick={() => handleSelectStore(store)} className={`w-full flex items-center gap-3 px-4 py-3 text-left smooth-transition active:bg-slate-200 ${isSelected ? 'bg-emerald-50/80' : 'hover:bg-slate-50'}`}>
                            <span className={`font-mono text-xs w-8 shrink-0 ${isSelected ? 'text-emerald-600 font-bold' : 'text-slate-400 font-bold'}`}>{store.code}</span><span className={`text-sm flex-1 ${isSelected ? 'text-emerald-800 font-bold' : 'text-slate-700 font-bold'}`}>{store.name}</span>{isSelected && <div className="w-2 h-2 bg-emerald-500 rounded-full shrink-0 shadow-sm" />}
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex-1 relative min-w-0" ref={partPanelRef}>
                  <button onClick={() => { if (hasParts) { setShowPartPanel(prev => !prev); setShowStorePanel(false); } }} className={`w-full flex items-center gap-1.5 px-3 py-1 rounded-xl text-[11px] smooth-transition active:scale-[0.98] border shadow-sm ${selectedPart ? 'bg-blue-50/90 border-blue-200/60 text-blue-800 hover:shadow-md' : hasParts ? 'bg-white/60 border-white/80 text-slate-500 hover:bg-white hover:shadow-md' : 'bg-slate-50/50 border-transparent text-slate-400 cursor-default shadow-none'}`}>
                    ...
                    {selectedPart ? <span className="font-semibold flex-1 text-left truncate">{selectedPart.name}<span className="text-blue-400 ml-1.5 text-[9px] bg-blue-100/50 px-1 py-0.5 rounded-md font-bold">{selectedPart.categories.length}개</span></span> : <span className="flex-1 font-bold text-left text-slate-400 text-[10px] truncate">{!selectedStore ? '영업점 먼저 선택' : hasParts ? '파트 선택' : '파트 없음'}</span>}
                    ...
                    <div className="absolute left-0 right-0 top-full mt-2 bg-white shadow-[0_20px_40px_rgba(0,0,0,0.15)] rounded-2xl border border-gray-200/50 z-[60] overflow-hidden origin-top animate-in fade-in slide-in-from-top-2">
                      <div className="max-h-[300px] overflow-y-auto custom-scrollbar py-1 bg-white">
                        {storeParts!.map((part, idx) => {
                          ...
                          return (
                            <button key={part.id} onClick={() => { onSelectPart?.(part.id); setShowPartPanel(false); }} className={`w-full flex items-center gap-3 px-4 py-3 text-left smooth-transition active:bg-slate-200 ${isSelected ? 'bg-blue-50/80' : 'hover:bg-slate-50'}`}>
                              <Layers size={14} className={isSelected ? 'text-blue-500' : 'text-slate-300'} /><span className={`font-mono text-xs w-7 shrink-0 ${isSelected ? 'text-blue-500 font-bold' : 'text-slate-400 font-bold'}`}>{String(idx + 1).padStart(3, '0')}</span><span className={`text-sm flex-1 ${isSelected ? 'text-blue-800 font-bold' : 'text-slate-700 font-bold'}`}>{part.name}</span><span className={`text-[10px] shrink-0 font-bold ${isSelected ? 'text-blue-500 bg-blue-100/50 px-1.5 py-0.5 rounded-md' : 'text-slate-400'}`}>{part.categories.length}개</span>{isSelected && <div className="w-2 h-2 bg-blue-500 rounded-full shrink-0 shadow-sm" />}
                            </button>
                          );
                        })}
                      </div>
