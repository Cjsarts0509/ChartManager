    ...
    <div className="glass-panel print:hidden relative z-50">
      <div className="px-3 py-2 flex justify-center">
        <div className="flex items-stretch gap-2">
            {/* 업로드 버튼 개선: 문구 제거 및 다른 버튼과 동일한 호버 액션 추가, 유채색 적용 */}
            <button 
              onClick={() => fileInputRef.current?.click()}
              className="border border-blue-200/50 rounded-xl px-5 flex flex-col items-center justify-center gap-1 bg-blue-50/80 hover:bg-blue-100 text-blue-700 smooth-transition active:scale-95 hover:-translate-y-1 hover:shadow-md cursor-pointer min-w-[140px] h-[60px]" 
            >
              <Upload size={22} className="mb-0.5" />
              <span className="text-[12px] font-bold whitespace-nowrap">엑셀 파일 업로드</span>
            ...
            <div className="flex flex-col gap-1.5 min-w-0 justify-center h-[60px]" style={{ flex: '0 1 380px' }}>
              <div onClick={() => handleDownloadCloudFile(thisWeekCloud?.url, thisWeekCloud?.filename)} className={`border shadow-sm rounded-xl px-3 py-1 bg-white/60 backdrop-blur-sm flex items-center gap-2 smooth-transition ${thisWeekCloud?.exists ? 'border-emerald-200 hover:bg-emerald-50 hover:scale-[1.02] cursor-pointer hover:shadow-md' : 'border-indigo-100 cursor-default'}`} title={thisWeekCloud?.exists ? "클릭하여 원본 엑셀 다운로드" : ""}>
                ...
                ) : <span className="text-xs font-bold text-indigo-400 truncate">{titleThisWeek || "파일 없음"}</span>}
              </div>

              <div onClick={() => handleDownloadCloudFile(lastWeekCloud?.url, lastWeekCloud?.filename)} className={`border shadow-sm rounded-xl px-3 py-1 bg-white/60 backdrop-blur-sm flex items-center gap-2 smooth-transition ${lastWeekCloud?.exists ? 'border-orange-200 hover:bg-orange-50 hover:scale-[1.02] cursor-pointer hover:shadow-md' : 'border-indigo-100 cursor-default'}`} title={lastWeekCloud?.exists ? "클릭하여 원본 엑셀 다운로드" : ""}>
                ...
                ) : <span className="text-xs font-bold text-indigo-400 truncate">{titleLastWeek || "파일 없음"}</span>}
              </div>
            </div>

            <div className="flex gap-1.5 shrink-0 h-[60px]">
              <InlineNoticePanel />
              <button onClick={onRefreshCloud} disabled={cloudLoading} className="flex flex-col items-center justify-center bg-teal-50/80 hover:bg-teal-100 text-teal-700 border border-teal-200/50 w-[72px] rounded-xl smooth-transition active:scale-95 hover:-translate-y-1 hover:shadow-md disabled:opacity-50"><RefreshCw size={22} className={`mb-1 ${cloudLoading ? 'animate-spin' : ''}`} /><span className="text-[11px] font-bold whitespace-nowrap">{cloudLoading ? '동기화중' : '클라우드'}</span></button>
              <a href={BOARD_URL} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center bg-sky-50/80 hover:bg-sky-100 text-sky-700 border border-sky-200/50 w-[72px] rounded-xl smooth-transition active:scale-95 hover:-translate-y-1 hover:shadow-md"><ExternalLink size={22} className="mb-1" /><span className="text-[11px] font-bold whitespace-nowrap">게시판</span></a>
              <button onClick={onAddList} className="flex flex-col items-center justify-center bg-indigo-50/80 hover:bg-indigo-100 text-indigo-700 border border-indigo-200/50 w-[72px] rounded-xl smooth-transition active:scale-95 hover:-translate-y-1 hover:shadow-md"><Plus size={22} className="mb-1" /><span className="text-[11px] font-bold whitespace-nowrap">페이지추가</span></button>
              <button onClick={onPrint} className="flex flex-col items-center justify-center bg-violet-50/80 hover:bg-violet-100 text-violet-700 border border-violet-200/50 w-[72px] rounded-xl smooth-transition active:scale-95 hover:-translate-y-1 hover:shadow-md"><Printer size={22} className="mb-1" /><span className="text-[11px] font-bold whitespace-nowrap">전체인쇄</span></button>
              ...
              <button onClick={onOpenCategoryConfig} className="flex flex-col items-center justify-center bg-rose-50/80 hover:bg-rose-100 text-rose-700 border border-rose-200/50 w-[72px] rounded-xl smooth-transition active:scale-95 hover:-translate-y-1 hover:shadow-md"><Settings size={22} className="mb-1" /><span className="text-[11px] font-bold whitespace-nowrap text-center">설정</span></button>
            </div>

            <div className="w-px bg-indigo-200/60 shrink-0 my-1 rounded-full ml-1" />
            ...
            <div className="flex flex-col gap-1 min-w-0 justify-center h-[60px]" style={{ flex: '0 1 360px' }}>
              <div className="relative" ref={storePanelRef}>
                <button onClick={() => { setShowStorePanel(prev => !prev); setShowPartPanel(false); }} className={`w-full flex items-center gap-1.5 px-3 py-1 rounded-xl text-[11px] smooth-transition active:scale-[0.98] border shadow-sm ${selectedStore ? 'bg-emerald-50/90 border-emerald-300/60 text-emerald-800 hover:shadow-md' : 'bg-indigo-50/60 border-indigo-200/80 text-indigo-600 hover:bg-indigo-100 hover:shadow-md'}`}>
                  <MapPin size={12} className={selectedStore ? 'text-emerald-600' : 'text-indigo-500'} />
                  {selectedStore ? <span className="font-bold flex-1 text-left truncate"><span className="text-emerald-600 font-mono mr-1.5">{selectedStore.code}</span>{selectedStore.name}</span> : <span className="flex-1 font-bold text-left text-indigo-500 text-[10px]">영업점 선택</span>}
                  {selectedStore && <span onClick={handleClearStore} className="p-1 rounded-full hover:bg-emerald-200/60 smooth-transition"><X size={10} className="text-emerald-700" /></span>}
                  <ChevronDown size={12} className={`transition-transform duration-300 shrink-0 ${showStorePanel ? 'rotate-180' : ''}`} />
                </button>
                {showStorePanel && (
                  <div className="glass-panel absolute left-0 right-0 top-full mt-2 bg-white/60 backdrop-blur-2xl border border-white/50 rounded-2xl shadow-xl z-[60] overflow-hidden origin-top animate-in fade-in slide-in-from-top-2">
                    <div className="p-2 border-b border-indigo-100/50">
                      <div className="flex items-center gap-2 bg-indigo-50/80 rounded-xl px-3 py-2 border border-indigo-100"><Search size={14} className="text-indigo-500 shrink-0" /><input type="text" value={storeSearch} onChange={(e) => setStoreSearch(e.target.value)} placeholder="코드/영업점명 검색" className="flex-1 bg-transparent text-sm font-bold outline-none placeholder-indigo-300 text-indigo-800" autoFocus />{storeSearch && <button onClick={() => setStoreSearch("")} className="p-0.5 hover:bg-indigo-200 rounded-full transition-colors"><X size={12} className="text-indigo-600" /></button>}</div>
                    </div>
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                      {filteredStores.map(store => {
                        const isSelected = selectedStore?.code === store.code;
                        return (
                          <button key={store.code} onClick={() => handleSelectStore(store)} className={`w-full flex items-center gap-3 px-4 py-3 text-left smooth-transition active:bg-indigo-200 ${isSelected ? 'bg-emerald-100/80' : 'hover:bg-indigo-50/80'}`}>
                            <span className={`font-mono text-xs w-8 shrink-0 ${isSelected ? 'text-emerald-700 font-bold' : 'text-indigo-400 font-bold'}`}>{store.code}</span><span className={`text-sm flex-1 font-bold ${isSelected ? 'text-emerald-900' : 'text-indigo-700'}`}>{store.name}</span>{isSelected && <div className="w-2 h-2 bg-emerald-600 rounded-full shrink-0 shadow-sm" />}
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex-1 relative min-w-0" ref={partPanelRef}>
                  <button onClick={() => { if (hasParts) { setShowPartPanel(prev => !prev); setShowStorePanel(false); } }} className={`w-full flex items-center gap-1.5 px-3 py-1 rounded-xl text-[11px] smooth-transition active:scale-[0.98] border shadow-sm ${selectedPart ? 'bg-blue-50/90 border-blue-300/60 text-blue-800 hover:shadow-md' : hasParts ? 'bg-indigo-50/60 border-indigo-200/80 text-indigo-600 hover:bg-indigo-100 hover:shadow-md' : 'bg-indigo-50/30 border-transparent text-indigo-400 cursor-default shadow-none'}`}>
                    <Layers size={12} className={selectedPart ? 'text-blue-600' : 'text-indigo-400'} />
                    {selectedPart ? <span className="font-bold flex-1 text-left truncate">{selectedPart.name}<span className="text-blue-500 ml-1.5 text-[9px] bg-blue-100 px-1 py-0.5 rounded-md font-bold">{selectedPart.categories.length}개</span></span> : <span className="flex-1 font-bold text-left text-indigo-500 text-[10px] truncate">{!selectedStore ? '영업점 먼저 선택' : hasParts ? '파트 선택' : '파트 없음'}</span>}
                    {selectedPart && <span onClick={(e) => { e.stopPropagation(); onSelectPart?.(null); }} className="p-1 rounded-full hover:bg-blue-200/60 transition-colors"><X size={10} className="text-blue-700" /></span>}
                    ...
                    <div className="glass-panel absolute left-0 right-0 top-full mt-2 bg-white/60 backdrop-blur-2xl border border-white/50 rounded-2xl shadow-xl z-[60] overflow-hidden origin-top animate-in fade-in slide-in-from-top-2">
                      <div className="max-h-[300px] overflow-y-auto custom-scrollbar py-1">
                        {storeParts!.map((part, idx) => {
                          ...
                          return (
                            <button key={part.id} onClick={() => { onSelectPart?.(part.id); setShowPartPanel(false); }} className={`w-full flex items-center gap-3 px-4 py-3 text-left smooth-transition active:bg-blue-200 ${isSelected ? 'bg-blue-100/80' : 'hover:bg-blue-50/50'}`}>
                              <Layers size={14} className={isSelected ? 'text-blue-600' : 'text-blue-300'} /><span className={`font-mono font-bold text-xs w-7 shrink-0 ${isSelected ? 'text-blue-700' : 'text-blue-400'}`}>{String(idx + 1).padStart(3, '0')}</span><span className={`text-sm flex-1 font-bold ${isSelected ? 'text-blue-900' : 'text-blue-700'}`}>{part.name}</span><span className={`text-[10px] font-bold shrink-0 ${isSelected ? 'text-blue-700 bg-blue-200 px-1.5 py-0.5 rounded-md' : 'text-blue-500'}`}>{part.categories.length}개</span>{isSelected && <div className="w-2 h-2 bg-blue-600 rounded-full shrink-0 shadow-sm" />}
                            </button>
                          );
                        })}
                      </div>
                ...
                <button onClick={onLoadPartLists} disabled={!selectedPart} className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] smooth-transition active:scale-95 border whitespace-nowrap shadow-sm ${selectedPart ? 'bg-blue-600 hover:bg-blue-700 hover:-translate-y-0.5 hover:shadow-md text-white border-blue-600 cursor-pointer' : 'bg-blue-100/50 text-blue-400 border-transparent cursor-not-allowed shadow-none'}`}><Download size={12} /><span className="font-bold">불러오기</span></button>
                <button onClick={onClearLists} className="flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] smooth-transition active:scale-95 border bg-rose-50 hover:bg-rose-100 hover:-translate-y-0.5 hover:shadow-md text-rose-600 hover:text-rose-700 border-rose-200 whitespace-nowrap shadow-sm"><Trash2 size={12} /><span className="font-bold">초기화</span></button>
              </div>
            </div>
        </div>
      </div>
