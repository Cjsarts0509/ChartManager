        ...
        {deleteTarget && (
          <div className="fixed inset-0 z-[260] flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel bg-white/95 rounded-2xl border border-red-200 w-[340px] overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="px-4 py-4 text-sm text-gray-700 space-y-2"><p><strong className="text-red-600">"{deleteTarget.name}"</strong> 파트를 삭제하시겠습니까?</p>{deleteTarget.categories.length > 0 && <p className="text-xs text-gray-500 bg-gray-50/80 rounded-xl p-2">적용된 <strong className="text-gray-700">{deleteTarget.categories.length}개</strong> 조코드가 함께 삭제됩니다.</p>}</div>
              <div className="px-4 py-3 border-t border-gray-200/50 bg-gray-50/50 flex justify-end gap-2"><button onClick={() => setDeleteTarget(null)} className="px-4 py-1.5 bg-gray-200/80 hover:bg-gray-300 text-gray-700 rounded-xl text-sm smooth-transition active:scale-95">취소</button><button onClick={handleDeleteConfirm} className="px-4 py-1.5 bg-red-600/90 text-white rounded-xl text-sm smooth-transition active:scale-95 shadow-sm">삭제</button></div>
            </motion.div>
          </div>
        )}
      </motion.div>
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      ...
      >
                  ...
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="glass-panel absolute left-0 right-0 top-full mt-1 bg-white/95 rounded-2xl z-[210] overflow-hidden shadow-lg">
                    ...
                    <div className="max-h-[200px] overflow-y-auto custom-scrollbar">
                      {filteredStores.map(store => (
                        <button key={store.code} onClick={() => { setSelectedStore(store); setShowStoreDropdown(false); setStoreSearch(''); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm smooth-transition ${selectedStore?.code === store.code ? 'bg-emerald-50/80' : 'hover:bg-gray-50'}`}>
        ...
        <div className="flex-1 flex overflow-hidden min-h-0 bg-white/80 backdrop-blur-sm">
          {loading ? (
            ...
            <>
              ...
              <div className="flex-1 flex flex-col min-w-0" style={{ flex: '1.2' }}>
                <div className="px-4 py-2 bg-blue-50/80 border-b border-blue-200/30 flex items-center justify-between shrink-0" onMouseDown={e => e.stopPropagation()}>
